...
namespace BISERP.Common
{
    ...
    {
        static readonly ILogger Logger = BISERPCommon.Logger.Register(typeof(AsyncWebCalls));
        ...
        public static async Task<T> GetAsync<T>(string url, CancellationToken cancellationToken) where T : new()
        {
            try
            {
                using (var httpClient = new HttpClient())
                {
                    ...
                    var weburl = BISERP.Common.Constants.WebAPIAddress + url + Common.Constants.JsonTypeResult;
                    httpClient.DefaultRequestHeaders.Add("Accept-Encoding", "deflate,gzip");
